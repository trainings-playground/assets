...
function Code({
  ...
  const isCopyFile = react.exports.useMemo(() => className.search(/file/) > -1, [className]);
  const onKeyup = ({
    target
  }) => {
    if (target.key === "Enter") {
  ...
  const cls = `${[isCmd ? "break-words training-command" : "", isCopyFile ? "training-copy-file" : ""].filter(Boolean).join(" ")}`;
  return /* @__PURE__ */ jsx("code", {
    role: "button",
    tabIndex: 0,
    onKeyUp: onKeyup,
    "data-cmd": isCmd ? className.substring("lang-".length) : null,
    "data-copy-file": isCopyFile ? className.substring("lang-".length) : null,
    className: cls,
    onClick: onClickCmd,
    children
  });
...
function Exercise({
  ...
  const _onClickCmd = ({
    ...
    var _a, _b;
    let terminal = (((_a = target.dataset) == null ? void 0 : _a.cmd) || "").replace(/command:/, "");
    let file;
    if (!terminal) {
      [, terminal, file] = ((_b = target.dataset) == null ? void 0 : _b.copyFile.match(/file:(.+)*:(.+)*/i)) || [];
    }
    onClickCmd(file ? {
      terminal,
      code: base64EncodeUnicode(target.innerText.trim()),
      file
    } : {
      terminal,
      cmd: target.innerText.trim()
    });
  };
  const onChange = ({
    target
...
export { Exercise as E, PropTypes as P, jsx as j };
